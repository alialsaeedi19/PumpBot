...
class PumpAndDumpDetector(EventListener):
    ...
    def onEvent(self, event: Event):
        if event.type == "ListingPriceUpdated":
            ...
            # BAD!!!
            if (
                self._lastUpdateTime.minute == currentDate.minute
                and self._lastUpdateTime.hour == currentDate.hour
            ):
                return
            ...
            (
                prices,
                volumes,
            ) = TrackedStockDatabase.getInstance().getMinuteStockPricesAndVolumes(
                event.data["Ticker"]
            )
            ...
            if probability >= self._classificationThreshold:
                ...
                EventDispatcher.getInstance().dispatchEvent(
                    PumpAndDumpEvent(event.data["Ticker"], currentPrice, probability)
                )
